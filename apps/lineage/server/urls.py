from django.urls import path
from . import views, api_service_views
from .accounts_views import *
from .config_views import *
from .tops_views import *
from .status_views import *
from .services_views import *

app_name = 'server'

urlpatterns = [
    path("api/players-online/", views.players_online, name="api_players_online"),
    path("api/top-pvp/", views.top_pvp, name="api_top_pvp"),
    path("api/top-pk/", views.top_pk, name="api_top_pk"),
    path("api/top-clan/", views.top_clan, name="api_top_clan"),
    path("api/top-rich/", views.top_rich, name="api_top_rich"),
    path("api/top-online/", views.top_online, name="api_top_online"),
    path("api/top-level/", views.top_level, name="api_top_level"),
    path("api/olympiad-ranking/", views.olympiad_ranking, name="api_olympiad_ranking"),
    path("api/olympiad-heroes/", views.olympiad_all_heroes, name="api_olympiad_all_heroes"),
    path("api/olympiad-current-heroes/", views.olympiad_current_heroes, name="api_olympiad_current_heroes"),
    path("api/grandboss-status/", views.grandboss_status, name="api_grandboss_status"),
    path("api/siege/", views.siege, name="api_siege"),
    path("api/siege-participants/<int:castle_id>/", views.siege_participants, name="api_siege_participants"),
    path("api/boss-jewel-locations/", views.boss_jewel_locations, name="api_boss_jewel_locations"),

    path("api/config/", api_config_panel, name="api_config_panel"),

    path('account/update-password', update_password, name='update_password'),
    path('account/dashboard', account_dashboard, name='account_dashboard'),
    path('account/register', register_lineage_account, name='lineage_register'),

    path('stats/top-pvp/', top_pvp_view, name='top_pvp'),
    path('stats/top-pk/', top_pk_view, name='top_pk'),
    path('stats/top-adena/', top_adena_view, name='top_adena'),
    path('stats/top-clans/', top_clans_view, name='top_clans'),
    path('stats/top-level/', top_level_view, name='top_level'),
    path("stats/top-online/", top_online_view, name="top_online"),

    path("siege-ranking/", siege_ranking_view, name="siege_ranking"),
    path('olympiad-ranking/', olympiad_ranking_view, name='olympiad_ranking'),
    path('olympiad-all-heroes/', olympiad_all_heroes_view, name='olympiad_all_heroes'),
    path('olympiad-current-heroes/', olympiad_current_heroes_view, name='olympiad_current_heroes'),
    path('boss_jewel_locations/', boss_jewel_locations_view, name='boss_jewel_locations'),
    path('status/grandboss/', grandboss_status_view, name='grandboss_status'),

    # account management views
    path('account/find-chars/', find_chars_view, name='find_chars'),
    path('account/check-char/', check_char_page, name='check_char'),
    path('account/check-name-exists/', check_name_exists_page, name='check_name_exists'),
    path('account/change-nickname/', change_nickname_view, name='change_nickname'),
    path('account/change-sex/', change_sex_view, name='change_sex'),
    path('account/unstuck/', unstuck_view, name='unstuck'),
    path('account/delete-skills/', delete_skills_view, name='delete_skills'),
    path('account/delete-skills-save/', delete_skills_save_view, name='delete_skills_save'),
    path('account/delete-hennas/', delete_hennas_view, name='delete_hennas'),
    path('account/delete-shortcuts/', delete_shortcuts_view, name='delete_shortcuts'),
    path('account/move-all-paperdoll/', move_all_paperdoll_view, name='move_all_paperdoll'),
    path('account/add-skills/', add_skills_view, name='add_skills'),
    path('account/update-class-in-olympiad/', update_class_in_olympiad_view, name='update_class_in_olympiad'),
    path('account/update-base-class/', update_base_class_view, name='update_base_class'),
    path('account/check-has-class-in-sub/', check_has_class_in_sub_view, name='check_has_class_in_sub'),

    # novas rotas de API de conta
    path('api/check-char/<str:acc>/<str:cid>/', api_service_views.check_char_view, name='api_check_char'),
    path('api/check-name-exists/', api_service_views.check_name_exists_view, name='api_check_name_exists'),
    path('api/change-nickname/', api_service_views.change_nickname_api, name='api_change_nickname'),
    path('api/change-sex/', api_service_views.change_sex_api, name='api_change_sex'),
    path('api/unstuck/', api_service_views.unstuck_api, name='api_unstuck'),
    path('api/delete-skills/<str:cid>/', api_service_views.delete_skills_api, name='api_delete_skills'),
    path('api/delete-skills-save/<str:cid>/', api_service_views.delete_skills_save_api, name='api_delete_skills_save'),
    path('api/delete-hennas/<str:cid>/', api_service_views.delete_hennas_api, name='api_delete_hennas'),
    path('api/delete-shortcuts/<str:cid>/', api_service_views.delete_shortcuts_api, name='api_delete_shortcuts'),
    path('api/move-all-paperdoll/<str:cid>/', api_service_views.move_all_paperdoll_api, name='api_move_all_paperdoll'),
    path('api/add-skills/', api_service_views.add_skills_api, name='api_add_skills'),
    path('api/update-class-in-olympiad/', api_service_views.update_class_in_olympiad_api, name='api_update_class_in_olympiad'),
    path('api/update-base-class/', api_service_views.update_base_class_api, name='api_update_base_class'),
    path('api/check-has-class-in-sub/', api_service_views.check_has_class_in_sub_api, name='api_check_has_class_in_sub'),
]
